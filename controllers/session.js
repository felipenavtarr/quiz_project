...
exports.checkLoginExpires = (req, res, next) => {
    ...
    if (req.session.loginExpires) { // Exists a user session
...
exports.adminOrMyselfRequired = (req, res, next) => {
    ...
    if (isAdmin || isMyself) {
        ...
        console.log('Forbidden route: it is not the logged in user, nor an administrator.');
        res.send(403);
    }
};
...
//   Strategies in Passport require a `verify` function, which accept
//   credentials (in this case, a token, tokenSecret, and Twitter
//   profile), and invoke a callback with a user object.
...
exports.create = passport.authenticate(
    'local',
    {
        ...
        failureFlash: 'Failed authentication. Please retry.'
    }
);
